defmodule TicTacToe.GameServer do
  ...
  def init(:ok) do
    ...
    if Enum.member?(:ets.all(), :game_in_progress) == false do
      :ets.new(:game_in_progress, [:public, :named_table])
    end

    :ets.insert(:game_in_progress, {game_id, grid})
  ...
  @impl true
  def handle_call(:get_grid, _from, game_id) do
    [head | tail] = :ets.lookup(:game_in_progress, game_id)
    {:reply, head, tail}
  end
  ...
  def get_grid(game_id) do
    GenServer.call(game_id, :get_grid)
  end
